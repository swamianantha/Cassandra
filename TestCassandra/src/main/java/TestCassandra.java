...
//		test.printTableNames();
		...
		//----------------------------
		int		count	= 2;
		test.writeData(CASS_TABLENAME, count);
		...
		// Delete table
		//--------------
		test.cassConn.dropTable (CASS_TABLENAME);
	...
	public void getCassVer () {
		ResultSet	rs	= cassConn.getSession().execute(GETVER_QUERY);
	    Row row = rs.one();
	...
	public void writeData (String tableName, int count) {
		...
		colVals.add(String.valueOf(count + 10000));
